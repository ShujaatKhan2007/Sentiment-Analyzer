...
#
# NOTE: Hugging Face's older "api-inference.huggingface.co" URL has been
# retired. Requests must now go through their "router" endpoint instead.
...
def analyze_sentiment(request: AnalyzeRequest):
    ...
    if response.status_code != 200:
        # Include Hugging Face's own error message (truncated) so it's
        # possible to diagnose issues like bad tokens, bad payloads, or
        # unsupported models directly from the API response, instead of
        # having to dig through server logs.
        raise HTTPException(
            status_code=502,
            detail=f"Hugging Face API returned an error (status {response.status_code}): {response.text[:300]}",
        )

    data = response.json()

    # The API returns either a nested list, e.g.:
    #   [[{"label": "POSITIVE", "score": 0.9998}, {"label": "NEGATIVE", "score": 0.0002}]]
    # or, occasionally, a flat list, e.g.:
    #   [{"label": "POSITIVE", "score": 0.9998}, {"label": "NEGATIVE", "score": 0.0002}]
    # so we handle both shapes here.
    try:
        scores = data[0]
        if isinstance(scores, dict):
            # It was actually a flat list, so data[0] is a single label/score
            # dict rather than a list of them. Use the full list instead.
            scores = data
